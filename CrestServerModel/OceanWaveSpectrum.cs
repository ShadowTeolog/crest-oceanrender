...
{
    public class OceanWaveSpectrum
    {
        ...
        public float GetAmplitude(float wavelength, float componentsPerOctave)
        {
            ...
            {
                Logger.Error("Wavelength must be >= 0f");
                return 0f;
            }
            ...
            {
                Logger.Error("Out of bounds index");
                return 0f;
            }
        ...
        /// </summary>
        public void GenerateWaveData(Random randomsource, int componentsPerOctave, ref float[] wavelengths, ref float[] anglesDeg)
        {
            ...
            for (var octave = 0; octave < NUM_OCTAVES; octave++)
            {
                for (var i = 0; i < componentsPerOctave; i++)
                {
                    ...
                    var  maxWavelengthi = Math.Min(minWavelengthi + invComponentsPerOctave * minWavelength, 2f * minWavelength);
                    wavelengths[index] = Mathf.Lerp(minWavelengthi, maxWavelengthi, (float) randomsource.NextDouble());

                    var rnd = (i + (float) randomsource.NextDouble()) * invComponentsPerOctave;
        ...
        public void ApplyPhillipsSpectrum(float windSpeed,Vector2 windDir,float absgravity)
        {
            for (int octave = 0; octave < NUM_OCTAVES; octave++)
            {
                float wl = SmallWavelength(octave) * 1.5f;
                var pow = PhillipsSpectrum(windSpeed, windDir, absgravity, Mathf.Pow(2f, SMALLEST_WL_POW_2), wl, 0f);
                // we store power on logarithmic scale. this does not include 0, we represent 0 as min value
                pow = Math.Max(pow, Mathf.Pow(10f, MIN_POWER_LOG));
                _powerLog[octave] = Mathf.Log10(pow);
            }
        }

        public void ApplyPiersonMoskowitzSpectrum(float windSpeed,float absgravity)
        {
            for (int octave = 0; octave < NUM_OCTAVES; octave++)
            {
                float wl = SmallWavelength(octave) * 1.5f;
                var pow = PiersonMoskowitzSpectrum(absgravity, windSpeed, wl);
                // we store power on logarithmic scale. this does not include 0, we represent 0 as min value
                pow = Math.Max(pow, Mathf.Pow(10f, MIN_POWER_LOG));
                _powerLog[octave] = Mathf.Log10(pow);
            }
        ...
        public void ApplyJONSWAPSpectrum(float windSpeed, float fetch,float absgravity)
        {
            for (int octave = 0; octave < NUM_OCTAVES; octave++)
            {
                float wl = SmallWavelength(octave) * 1.5f;
                var pow = JONSWAPSpectrum(absgravity, windSpeed, wl, fetch);
                // we store power on logarithmic scale. this does not include 0, we represent 0 as min value
                pow = Math.Max(pow, Mathf.Pow(10f, MIN_POWER_LOG));
                _powerLog[octave] = Mathf.Log10(pow);
            }
